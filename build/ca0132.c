 ...
 */
/* AE-9 ACM module I2C register offsets within CA0132 BAR2 (spec->mem_base) */
/* AE-9 ACM I2C register offsets on BAR2 (spec->mem_base) */
...
static int ae9_acm_init(struct hda_codec *codec)
{
	...
	codec_info(codec, "AE-9: Starting ACM init (BAR2)\n");
	status = readl(spec->mem_base + AE9_I2C_STATUS);
	codec_info(codec, "AE-9 I2C status: 0x%02x\n", status & 0xff);

	/*
	 * GPIO init in ca0132_mmio_init_ae5() triggers the ACM to
	 * respond on the I2C bus. By the time we get here (after DSP
	 * download), the response is already in the FIFO. Status 0x03
	 * means the ACM has responded and is waiting for acknowledge.
	 *
	 * If status is NOT 0x03, the bus needs a kick-start with 0x30.
	 */
	if ((status & 0xff) != 0x03) {
		codec_info(codec, "AE-9: Bus not ready, sending init\n");
		...
		codec_info(codec, "AE-9 post-init: 0x%02x (%d ms)\n",
			   status & 0xff, i);
	}

	/* Drain FIFO (5 bytes expected: 01,f1,01,c7,c1) */
	for (i = 0; i < 5; i++) {
		fifo = readl(spec->mem_base + AE9_I2C_FIFO);
		codec_info(codec, "AE-9 FIFO[%d]: 0x%02x\n",
			   i, fifo & 0xff);
	}

	/*
	 * Acknowledge ACM response: exact Windows sequence.
	 * Write 0x80 to DATA, then CMD 0x0d, then DATA 0x00.
	 */
	writel(0x80, spec->mem_base + AE9_I2C_DATA);
	status = readl(spec->mem_base + AE9_I2C_STATUS);
	codec_info(codec, "AE-9 post-ack status: 0x%02x\n", status & 0xff);
	...
	status = readl(spec->mem_base + AE9_I2C_STATUS);
	codec_info(codec, "AE-9 post-0x0d status: 0x%02x\n", status & 0xff);

	/* Phase 2: Reset + Handshake + Key verification */
	...
	/* Phase 4: Identification + Output routing + DAC */
...
static void ca0132_mmio_init_ae5(struct hda_codec *codec)
{
	...
	for (i = 0; i < count; i++) {
		 ...
		 * they write a different value to 0x20c.
		 */
...
static int ca0132_codec_probe(struct hda_codec *codec, const struct hda_device_id *id)
{
	...
	/* Skip second codec on AE-5/AE-7/AE-9 cards - only D1 is used */
	if (codec->addr >= 2) {
		codec_dbg(codec, "ca0132: skipping secondary codec at addr %d\n", codec->addr);
		return -ENODEV;
	}
