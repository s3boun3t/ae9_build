...
/*
 * AE-9 MMIO output presets — 4 commands (not 6 like AE-5/AE-7).
 * Source: patch 0008 (Connor McAdams).
 * group/target/vals confirmed from Windows INF analysis.
 */
static const struct ae_ca0113_output_set ae9_ca0113_output_presets = {
	.group  = { 0x48, 0x48, 0x48, 0x48, 0x00, 0x00 },
	.target = { 0x1d, 0x0d, 0x17, 0x19, 0x00, 0x00 },
		    /* Speakers. */
	.vals   = { { 0x40, 0x00, 0x00, 0x00, 0x00, 0x00 },
		    /* Headphones. */
...
struct ca0132_spec {
	...
	/* AE-9: hrtimer for period wakeup (DSP firmware suppresses HDA IRQs) */
	struct hrtimer ae9_period_timer;
	struct snd_pcm_substream *ae9_playback_substream;
	struct hdac_stream *ae9_hstr;		/* HDA DMA stream for IOCE clear */
	ktime_t ae9_period_time;

	/*
	 * AE-9: ES9038 keepalive timer.
	 *
	 * VFIO traces show Windows sends a fixed 6-write sequence to BAR2
	 * registers 0x20c/0x210 every ~16ms throughout playback.  Without
	 * this keepalive the ES9038 DAC stays silent.
	 */
	struct hrtimer ae9_keepalive_timer;
...
/*
 * AE-9 pin configurations from Creative OEM INF (CtxHda.inf).
 * Source: PinConfig\11020071 section, verified against hardware.
 *
 * Key differences from AE-7:
 *  - 0x0c: SPDIF Out not connected (0x414510f0 vs 0x014510f0)
 *  - 0x11: Port B BackPanel HP (0x41a170ff vs 0x018170ff)
 *  - 0x13: What U Hear In (same as AE-7: 0x908700f0)
 */
static const struct hda_pintbl ae9_pincfgs[] = {
	/*
	 * Pin configs for AE-9 (SSID 11020071).
	 *
	 * NID 0x0F is the ACM headphone jack — must be declared as
	 * HP Out in a separate association (Assoc=2) so PipeWire/
	 * PulseAudio exposes it as a distinct "Headphone" output.
	 * NID 0x0B is the rear Line Out (Assoc=1).
	 * NID 0x10 is center/LFE or front panel HP (Assoc=1).
	 */
	{ 0x0b, 0x01014010 }, /* Port G: Line Out Rear (Assoc=1 Seq=0) */
	{ 0x0c, 0x414510f0 }, /* SPDIF Out: Not connected */
	{ 0x0d, 0x414510f0 }, /* SPDIF Out 2: not connected */
	{ 0x0e, 0x01c520f0 }, /* SPDIF In */
	{ 0x0f, 0x0221401f }, /* Port A: HP Out Front (Assoc=2 Seq=f) */
	{ 0x10, 0x02216011 }, /* Port D: HP Out Front (Assoc=1 Seq=1) */
	{ 0x11, 0x41a170ff }, /* Port B: not-connected */
	{ 0x12, 0x01a170f0 }, /* Port C: LineMicIn1 */
	{ 0x13, 0x908700f0 }, /* What U Hear In */
	{ 0x18, 0x500000f0 }, /* N/A */
	{}
};
		...
		unsigned int len, void *reply, unsigned int *reply_len)
{
	...
	if (status < 0) {
		codec_dbg(codec, "dspio_scp: send scp msg failed (mod=0x%02x req=0x%02x dir=%d)\n", mod_id, req, dir);
		return status;
	}
 ...
 */
/*
 * Wait for CA0113 command buffer ready (bit 23 = 0).
 * Returns 0 on success, -ETIMEDOUT if busy after 2ms.
 */
static int ca0113_mmio_wait_ready(struct ca0132_spec *spec)
{
	unsigned int ctrl;
	int timeout = 200;
...
/*
 * Wait for CA0113 response ready (bit 23 = 1).
 * Returns 0 on success, -ETIMEDOUT after 50ms.
 */
static int ca0113_mmio_wait_response(struct ca0132_spec *spec)
{
	unsigned int ctrl;
	int timeout = 500;
...
/*
 * CA0113 MMIO command protocol (Fernando RE / VFIO-verified).
 *
 * The CA0113 command interface at BAR2+0x200 requires:
 *   1. Wait for bit 23 of 0x20c = 0 (not busy)
 *   2. Write codec address in bits 1-2 of 0x20c
 *   3. Write group to 0x804
 *   4. Write target|value to 0x204 (CODEC_DATA)
 *   5. Set bit 0 of 0x20c (trigger command execution)
 *   6. Wait for bit 23 of 0x20c = 1 (response ready)
 *
 * Connor's original code wrote 0x800005 directly to 0x20c, setting
 * bit 23 from software rather than waiting for hardware to set it.
 * This worked on AE-5/AE-7 but not on AE-9 where the hardware
 * enforces the protocol strictly (0x208 stays 0xffffffff).
 */
 ...
 * Same Fernando protocol but with type2 (bit 1) instead of type1 (bit 2).
 */
 ...
 * PCM callbacks
 */
/* Forward declarations — defined later with ae9_playback_pcm_open */
static void ae9_period_timer_start(struct ca0132_spec *spec,
				   struct snd_pcm_substream *substream);
static void ae9_period_timer_stop(struct ca0132_spec *spec);
static void ae9_keepalive_start(struct ca0132_spec *spec);
static void ae9_keepalive_stop(struct ca0132_spec *spec);
			...
			struct snd_pcm_substream *substream)
{
	...
	if (ca0132_quirk(spec) == QUIRK_AE9) {
		 ...
		 *
		 * ae9_playback_pcm_open sets channels 2-6, S16/S24/S32.
		 * snd_hda_codec_setup_stream already sent the correct
		 * format to both the SD and the codec.
		 *
		...
		msleep(20);
		/*
		 * AE-9 Normal Mode pcm_prepare.
		 * Decoded from Windows CORB captures in Normal Mode
		 * (non-Direct, non-SBX) on 8 avril 2026.
		 */
        
        /* dac2port = headphone (0xA1) */
		chipio_set_control_param_no_mutex(codec, 0x0d, 0xa1);

		/*
		 * AE-9 pcm_prepare: Normal Mode playback cycle.
		 * Decoded from corb_norm_boot_t17/t22 (8 avril 2026).
		 *
		 * Normal Mode (non-Direct, non-SBX) uses 6ch format and
		 * routes through the DSP pipeline. The DSP firmware
		 * expects 6ch input to activate its output DMA channels.
		 *
		 * Stream 0x05: src=0x43, dst=0x00 (internal DSP routing)
		 * Stream 0x18: src=0x09, dst=0xd0, 6ch (DSP→DAC output)
		 */

		/* Configure stream 0x05 (HDA→DSP): dst=0x00 (NOT 0xd0!) */
		chipio_set_control_param_no_mutex(codec,
		...
		/* Configure stream 0x18 (DSP→DAC): 6ch output */
		chipio_set_control_param_no_mutex(codec,
			CONTROL_PARAM_STREAM_ID, 0x18);
		chipio_set_control_param_no_mutex(codec,
			CONTROL_PARAM_STREAM_SOURCE_CONN_POINT, 0x09);
		chipio_set_control_param_no_mutex(codec,
			CONTROL_PARAM_STREAM_DEST_CONN_POINT, 0xd0);

		/* Connection rates for stream 0x18 output */
		chipio_set_control_param_no_mutex(codec, 0x1d, 0xd0);
		chipio_set_control_param_no_mutex(codec, 0x1e, 0x0b);

		/* Stream 0x18: 6 channels + param 0x1B = 6 */
		chipio_set_control_param_no_mutex(codec,
			CONTROL_PARAM_STREAM_ID, 0x18);
		chipio_set_control_param_no_mutex(codec, 0x1b, 0x06);

		/* PLL register 0x24 = 0x08, PMU write = 0x00 */
		snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0, 0x70d, 0x24);
		snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0, 0x70e, 0x08);
		snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0,
				    VENDOR_CHIPIO_PLL_PMU_WRITE, 0x00);
		...
		/* ASI toggle: 0x08 then later 0x0f */
		chipio_set_control_param_no_mutex(codec,
			CONTROL_PARAM_ASI, 0x08);

		/* HCI memory writes (Connor values, confirmed in CORB) */
		...
		/*
		 * NID 0x02: force 6ch format (0x0035 = 48kHz/32bit/6ch).
		 * Windows Normal Mode CORB shows 0x0035, not 0x0031 (2ch).
		 * The DSP firmware expects 6ch input to activate its
		 * output DMA pipeline. Without 6ch, ACTIVE stays 0.
		 */
		/* CORB t32: Windows writes 0x18B008=0xF0 just before stream format */
		chipio_write(codec, 0x18b008, 0x000000f0);
		snd_hda_codec_write(codec, 0x02, 0,
				    AC_VERB_SET_STREAM_FORMAT, 0x0035);
		/* dac2port = headphone (repeat after format change) */
		chipio_set_control_param(codec, 0x0d, 0xa1);

		/* Pin config: disable unused, enable HP */
		...
		/* NID 0x02 stream tag=1 + unmute amps */
		snd_hda_codec_write(codec, 0x02, 0,
				    AC_VERB_SET_CHANNEL_STREAMID,
				    (stream_tag << 4) | 0);
		snd_hda_codec_write(codec, 0x02, 0,
		...
		/* NID 0x07 input mixer amps (Normal Mode CORB t17) */
		snd_hda_codec_write(codec, 0x07, 0, 0x360, 0x52);
		snd_hda_codec_write(codec, 0x07, 0, 0x350, 0x52);

		/* Start period timer + keepalive */
...

		{
			...
			unsigned int cfg0 = 0, cfg1 = 0, cfg2 = 0;
			unsigned int cstat = 0, cprop = 0;

			chipio_read(codec, 0x110F00, &cfg0);
			chipio_read(codec, 0x110F10, &cfg1);
			chipio_read(codec, 0x110F20, &cfg2);
			chipio_read(codec, 0x110FF4, &cstat);
			chipio_read(codec, 0x110FF8, &cprop);
			chipio_read(codec, 0x110F08, &ch0);
			chipio_read(codec, 0x110F18, &ch1);
			chipio_read(codec, 0x110F28, &ch2);
			codec_info(codec,
				   "AE-9 pcm_prepare: done ch0=%08x ch1=%08x ch2=%08x cfg0=%08x cfg1=%08x cfg2=%08x cstat=%08x cprop=%08x\n",
				   ch0, ch1, ch2, cfg0, cfg1, cfg2, cstat, cprop);
		}
	}
			...
			struct snd_pcm_substream *substream)
{
	...
	if (ca0132_quirk(spec) == QUIRK_AE9) {
		...
		/* keepalive stays running — stopped only at codec_free */
	}
...
static void ae5_mmio_select_out(struct hda_codec *codec)
{
	...
	if (ca0132_quirk(spec) == QUIRK_AE5) {
		...
		/* AE-9 uses only 4 MMIO commands (patch 0008). */
...
static int ca0132_alt_select_out_quirk_set(struct hda_codec *codec)
{
	...
	codec_dbg(codec, "AE-9 select_out: dac2port=0x%02x\n",
		  out_info->dac2port);
...
static int ca0132_alt_select_out(struct hda_codec *codec)
{
	...
	codec_dbg(codec, "%s\n", __func__);
	codec_dbg(codec, "AE-9 select_out: enter, dsp_state=%d\n",
		  spec->dsp_state);
	...
	if (ca0132_quirk(spec) == QUIRK_AE9) {
		/*
		 * AE-9: headphones live on the external ACM breakout box.
		 * The CA0132 jack-sense pins cannot detect insertion there,
		 * so auto-detect always yields SPEAKER_OUT (wrong).
		 * Force HEADPHONE_OUT unconditionally.
		 */
		spec->cur_out_type = HEADPHONE_OUT;
	} else if (auto_jack) {
		jack_present = snd_hda_jack_detect(codec, spec->unsol_tag_hp) ||
	...
	codec_dbg(codec, "AE-9 select_out: MUTE err=%d, cur_out=%d\n",
		  err, spec->cur_out_type);
	if (err < 0)
		goto exit;

	if (ca0132_alt_select_out_quirk_set(codec) < 0)
		goto exit;
	codec_dbg(codec, "AE-9 select_out: quirk_set done\n");
	...
	case HEADPHONE_OUT:
		...
		if (ca0132_quirk(spec) == QUIRK_AE9) {
			/*
			 * AE-9: the ACM board has no jack-sense hardware on
			 * NID 0x0f (jack_detect always returns 0). Always
			 * route to out_pins[1] (0x0f = ACM headphone jack).
			 */
			headphone_nid = spec->out_pins[2]; /* AE-9: Windows CORB t32 shows NID 0x10 (out_pins[2]) active */
		} else if (snd_hda_jack_detect(codec, spec->unsol_tag_front_hp)) {
		...
		{
			unsigned int _pc = snd_hda_codec_read(codec,
					headphone_nid, 0,
					AC_VERB_GET_PIN_WIDGET_CONTROL, 0);
			codec_dbg(codec,
				  "AE-9 select_out: pin 0x%02x pin-ctls=0x%02x\n",
				  headphone_nid, _pc);
		}

		/*
		 * MASTERCONTROL output mode.
		 * FLOAT_ZERO = no effects (standard path, PE=0)
		 * FLOAT_ONE = SBX pipeline active
		 * No AE-9 override — follow same path as all other cards.
		 */
		if (outfx_set)
...
static void ca0132_unsol_hp_delayed(struct work_struct *work)
{
	...
	/*
	 * AE-9: ignore HP jack events until the DSP is fully initialised.
	 * select_out calls dspio_set_uint_param(0x96, ...) which timeouts
	 * if the DSP output effects module is not yet ready, causing the
	 * DSP to remain permanently muted.
	 */
	if (ca0132_quirk(spec) == QUIRK_AE9 &&
...
static int ca0132_alt_select_in(struct hda_codec *codec)
{
	...
	case REAR_MIC:
		...
		case QUIRK_AE9:
			ca0113_mmio_command_set(codec, 0x30, 0x28, 0x00);
			tmp = FLOAT_ONE; /* Normal mode (Windows CORB) */
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	/*
	 * The AE-7 and AE-9 have no front microphone, limit items to 2:
	 * rear mic and line-in.
	 */
	if (ca0132_quirk(spec) == QUIRK_AE7 ||
...
/* Forward declarations for ZxR DB Pro functions defined later. */
...
static int ca0132_build_controls(struct hda_codec *codec)
{
	...
	/* D2 ACM-only codec: no mixer controls */
	if (spec->is_d2_acm_only)
		return 0;

	/* ZxR DB Pro board has a completely different control set. */
...
/*
 * AE-9 playback open: set hardware constraints.
 *
 * NID 0x02 reports no PCM capabilities (param 0x06 = 0x0) because the
 * CA0132 DSP handles all sample-rate conversion. Without explicit
 * constraints the HDA core leaves buffer_size_max at 1048576 frames,
 * causing a 4 MB DMA allocation that fails with -ENOMEM before the
 * stream can start. Cap buffer and period sizes to sensible values.
 */
/*
 * AE-9 ES9038 keepalive timer.
 *
 * VFIO capture shows Windows sends a full I2C keepalive cycle to the ACM
 * every ~500ms.  The cycle includes handshake, config refresh, DSP routing,
 * headphone amp enable, and DAC channel enable packets.  Without this cycle
 * the ACM may deactivate the DAC/amp and produce silence.
 *
 * The 16ms CA0113 0x20c/0x210 sub-cycle is also maintained for DSP sync.
 *
 * Because hrtimer callbacks run in atomic context, we use a stripped-down
...
static enum hrtimer_restart ae9_keepalive_callback(struct hrtimer *hrt)
{
...
/*
 * AE-9 hrtimer for period wakeup.
 *
 * The CA0132 DSP firmware intercepts the HDA DMA engine and does not
 * propagate BDL IOC interrupts reliably.  This timer fires at each
 * nominal period boundary and calls snd_pcm_period_elapsed() so that
 * userspace can refill the buffer at the correct cadence.
 *
 * It also clears IOCE (bit 2 of SD_CTL) which stalls the DSP DMA.
 */
static enum hrtimer_restart ae9_period_timer_callback(struct hrtimer *hrt)
{
	struct ca0132_spec *spec =
		container_of(hrt, struct ca0132_spec, ae9_period_timer);
	struct snd_pcm_substream *substream = spec->ae9_playback_substream;

	if (!substream || !substream->runtime)
		return HRTIMER_NORESTART;

	if (substream->runtime->status->state != SNDRV_PCM_STATE_RUNNING) {
		/* Not yet triggered — keep polling quickly */
		hrtimer_forward_now(hrt, ns_to_ktime(1000000ULL));
		return HRTIMER_RESTART;
	}

	/* Clear IOCE (bit 2) if set — it stalls the DSP DMA engine */
	if (spec->ae9_hstr && spec->ae9_hstr->sd_addr) {
		u8 ctl = readb(spec->ae9_hstr->sd_addr);

		if (ctl & 0x04)
			writeb(ctl & ~0x04, spec->ae9_hstr->sd_addr);
	}

	snd_pcm_period_elapsed(substream);

	hrtimer_forward_now(hrt, spec->ae9_period_time);
	return HRTIMER_RESTART;
}
				   ...
				   struct snd_pcm_substream *substream)
{
	...
	/*
	 * AE-9: cancel any running timer and update state for the new prepare.
	 * pcm_prepare is called multiple times during format probing — each
	 * call cancels the previous timer and starts a new one.  The callback
	 * checks SNDRV_PCM_STATE_RUNNING and stops itself if the DMA is not
	 * yet active, so firing during format probing is harmless.
	 */
	hrtimer_cancel(&spec->ae9_period_timer);

	spec->ae9_playback_substream = substream;
	spec->ae9_hstr = (struct hdac_stream *)runtime->private_data;

	period_ns = div_u64((u64)runtime->period_size * NSEC_PER_SEC,
			    runtime->rate);
	spec->ae9_period_time = ns_to_ktime(period_ns);

	hrtimer_start(&spec->ae9_period_timer, ns_to_ktime(1000000ULL),
		      HRTIMER_MODE_REL);
}
...
{
	hrtimer_cancel(&spec->ae9_period_timer);
	spec->ae9_playback_substream = NULL;
	spec->ae9_hstr = NULL;
}

static int ae9_playback_pcm_open(struct hda_pcm_stream *hinfo,
				 struct hda_codec *codec,
				 struct snd_pcm_substream *substream)
{
...
static const struct hda_pcm_stream ae9_pcm_analog_playback = {
	.substreams = 1,
	.channels_min = 2,
	.channels_max = 6,
	.ops = {
		.open    = ae9_playback_pcm_open,
		.prepare = ca0132_playback_pcm_prepare,
		.cleanup = ca0132_playback_pcm_cleanup,
		.get_delay = ca0132_playback_pcm_delay,
	},
...
static int ca0132_build_pcms(struct hda_codec *codec)
{
	...
	/* D2 ACM-only codec: no PCM streams */
	if (spec->is_d2_acm_only)
		return 0;

	/* ZxR DB Pro board has a different PCM configuration. */
	if (ca0132_quirk(spec) == QUIRK_ZXR_DBPRO)
	...
	/*
	 * AE-9: use a dedicated stream descriptor with an .open callback
	 * that sets hardware constraints (rates, formats, buffer limits).
	 * NID 0x02 reports no PCM caps so the HDA core cannot derive them
	 * from hardware; without buffer_bytes_max the allocation reaches
	 * 4 MB and fails with -ENOMEM before the stream can start.
	 */
	if (ca0132_quirk(spec) == QUIRK_AE9)
		info->stream[SNDRV_PCM_STREAM_PLAYBACK] = ae9_pcm_analog_playback;
	else
...
{
	/*
	 * AE-9: use the same 3 DSP streams as all other CA0132 cards.
	 *
	 * The CA0132 DSP allocates internal DMA channels sequentially when
	 * streams are started. All cards (SBZ, AE-5, AE-7) start 0x0c, 0x03,
	 * 0x04 in that order — giving the DSP channels 0/1/2. With all 3
	 * allocated, the DSP activates its audio processing pipeline.
	 *
	 * The previous AE-9 override started only 0x0c, leaving channels 1+2
	 * unallocated. Per the comment below, "issues can arise if streams
...
/*
 * AE-9 post-DSP setup functions.
 * Source: patch 0007 by Connor McAdams.
 */
				      ...
				      bool start_stream)
{
	...
	if (start_stream) {
		/*
		 * AE-9: Start stream 0x05 (HDA→DSP) BEFORE stream 0x18.
		 * Without this, exram stays 0xff and audio data never
		 * reaches the DSP. Stream 0x18 (DSP→I2S) is started
		 * below after the CA0113 commands.
		 */
...
static void ae9_post_dsp_asi_setup(struct hda_codec *codec)
{
	...
	/*
	 * AE-9: ASI initialization cycle — adapted from AE-7.
	 * SET_PROC_STATE 0x83 confirmed in CORB t12 index 106.
	 * ASI=0 reset + exram 0xFA92 RET patch used by AE-5, AE-7, ZxR.
	 * Without the 0xFA92 patch, the 8051 overrides ASI back to 0x08.
	 */
	chipio_set_control_flag(codec, CONTROL_FLAG_ASI_96KHZ, 1);
	snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0, 0x724, 0x83);
	chipio_set_control_param(codec, CONTROL_PARAM_ASI, 0);
	...
	ca0113_mmio_command_set(codec, 0x48, 0x0f, 0x00);
	/*
	 * AE-9: Windows writes 0x31 here (not 0x37).
	 * 0x31 is the volume/filter preset for the AE-9.
	 */
	...
	ca0113_mmio_command_set(codec, 0x49, 0x0a, 0x07);
	/* GPIO4 removed — must be set AFTER ACM init (see ae9_setup_defaults) */
	/*
	 * AE-9: Windows trace confirms type2(0x48,0x07,0x00) then
	 * type1(0x48,0x07,0x80) here (not type2 0x80).
	 */
	ca0113_mmio_command_set_type2(codec, 0x48, 0x07, 0x00);
	ca0113_mmio_command_set(codec, 0x48, 0x07, 0x80);
	...
	/*
	 * AE-9: Windows trace shows type2(0x48,0x07,0x00) then
	 * type1(0x48,0x07,0x00) — both zero. Then type2(0x49,0x0e,0x00)
	 * followed by type1(0x49,0x0e,0x01).
	 * Volume regs 0x0f/0x10 get value 0x1e (not 0x37).
	 */
	ca0113_mmio_command_set_type2(codec, 0x48, 0x07, 0x00);
	...
	/* GPIO4 removed — must be set AFTER ACM init */
...
static int ae9_acm_bus_init(struct hda_codec *c)
{
	 ...
	 * Windows VFIO trace shows ZERO reads on this range.
	 * Reads may be interpreted as commands by the I2C controller.
	 */
	writel(0x80, base + 0xC0C);		/* STATUS: assert reset */
	writel(0x30, base + 0xC00);		/* CMD: clock/bus enable */
	writel(0x00, base + 0xC04);		/* DATA: kick */
	writel(0x00, base + 0xC0C);		/* STATUS: release reset */
	writel(0x03, base + 0xC0C);		/* STATUS: bus ready (×3) */
	writel(0x03, base + 0xC0C);
	writel(0x03, base + 0xC0C);

	/* FIFO load — no reads between writes */
	...
	/* 0x1c pulse — I2S clock reset (outside 0xC00-0xC7F range) */
	writel(0x00000480, base + 0x01c);
	writel(0x00880480, base + 0x01c);

	/* Signal MCU up + finalize */
	writel(0x83, base + 0xC0C);
	writel(0x0d, base + 0xC00);
	writel(0x00, base + 0xC04);
	writel(0x03, base + 0xC0C);

	/* GPIO5 before first packet */
	writew(0x0105, base + 0x320);

	/* TX#1 tail: MCU reset packet — no reads */
	writel(0xf0, base + 0xC00);
			  ...
			  const u8 *data, int len)
{
	...
	/*
	 * Phase 1: No readl() on 0xC00-0xC7F — Windows never reads
	 * this range. Reads may interfere with the I2C controller.
	 */
	writel(0xf0, base + 0xC00);
	for (i = 0; i < len; i++)
		writel(data[i], base + 0xC00);
...
static void ae9_acm_init(struct hda_codec *c)
{
	 ...
	 * Reference: {0x03,0x03,0x02,0x40,0x40}
	 * Byte 4 = 0x40 = SBX OFF. Was incorrectly 0x00 = SBX ON.
	 */
 ...
 * Shares the AE-7 DSP setup, then additionally initialises the
 * ACM breakout module over the CA0132's I2C bridge (BAR2 + 0xC00).
 */
static void ae9_setup_defaults(struct hda_codec *codec)
{
	struct ca0132_spec *spec = codec->spec;
	...
	if (spec->dsp_state != DSP_DOWNLOADED) {
		codec_warn(codec, "AE-9: DSP not downloaded (state=%d), aborting\n",
			   spec->dsp_state);
		return;
	}
	...
	if (ca0132_quirk(spec) == QUIRK_AE9) {
		...
		/* DIAG: is 8051 alive after handshake? */
		{
			unsigned int st = snd_hda_codec_read(codec,
				WIDGET_CHIP_CTRL, 0, 0xF01, 0);
			unsigned int dp = snd_hda_codec_read(codec,
				WIDGET_CHIP_CTRL, 0, 0xF10, 0x0D);
			codec_info(codec,
		 ...
		 *
		 * VFIO trace shows Windows sends these 10 commands
		 * (with sync) at every playback start. We must send
		 * them here — the only window where 0x860=1.
		 * After handshake_done, HDA verbs set bit 16 and
		 * CA0113 group 0x48/0x49 commands stop reaching DAC.
		 */
		ca0113_mmio_command_set(codec, 0x48, 0x1d, 0x80);
		ca0113_mmio_command_set(codec, 0x48, 0x0d, 0x40);
		...
		/* DIAG: is 8051 alive after early DAC? */
		{
			unsigned int st = snd_hda_codec_read(codec,
				WIDGET_CHIP_CTRL, 0, 0xF01, 0);
			codec_info(codec,
	...
	/*
	 * AE-9: ChipIO params from Windows CORB playback trace.
	 * Sent during Phase 3 of Windows playback — configure I2S
	 * output routing to ES9038Q2M DAC in the ACM.
	 * Without these, DSP does not route audio to I2S output.
	 * See ae9_corb_boot_analysis.md
	 */
	chipio_set_control_param(codec, 0x7D, 0x15);
	/*
	 * AE-9: Router writes from Windows CORB boot dump Phase 3.
	 * These configure the audio path to the headphone DAC.
	 * Without them, the DSP has no route to the output.
	 */
	chipio_write(codec, 0x190060, 0x00014000);
	chipio_set_control_param(codec, 0x90, 0x14);
	chipio_write(codec, 0x190064, 0x00014101);
	chipio_set_control_param(codec, 0x91, 0x14);
	chipio_write(codec, 0x19042C, 0x00000001);
	chipio_set_control_param(codec, 0xFC, 0x07);

	/*
	 * AE-9: Windows CORB boot Phase 7 — params 0x18 and 0x1C.
	 *
	 * These are sent after the router writes and before ASI=0x0F.
	 * Param 0x18 = 0x14 (purpose unknown, possibly I2S output config).
	 * Param 0x1C = 1 then later 0 (routing commit toggle).
	 * Without these, the DSP I2S output pipeline may not activate.
	 * Discovered in CORB capture 28 mars 2026.
	 */
	chipio_set_control_param(codec, 0x18, 0x14);
	chipio_set_control_param(codec, 0x1C, 0x01);
	 ...
	 * audio reaches the I2S bus to the ES9038Q2M DAC.
	 */
	tmp = FLOAT_ZERO;
	 ...
	 */
	/* Module 0x80 (MASTERCONTROL) Normal Mode — from CORB t12 decode */
	dspio_set_uint_param(codec, 0x80, 0x01, FLOAT_ZERO);  /* CORB #1 */
	dspio_set_uint_param(codec, 0x80, 0x0D, FLOAT_ZERO);  /* CORB #2 */
	dspio_set_uint_param(codec, 0x80, 0x0E, FLOAT_ZERO);  /* CORB #3 */
	dspio_set_uint_param(codec, 0x80, 0x00, FLOAT_ONE);   /* CORB #4: master mode=Normal */
	dspio_set_uint_param(codec, 0x80, 0x05, FLOAT_ONE);   /* CORB #5 */
	...
	/*
	 * AE-9 Normal Mode: PARAM 0x01=1 and PARAM 0x03=3.
	 * Discovered in corb_norm_boot_t12.txt (8 avril 2026).
	 * These are NEVER present in Direct Mode captures and were
	 * never sent by our driver. Purpose unknown but they appear
	 * in every Normal Mode boot right after the SCP trigger.
	 */
	chipio_set_control_param(codec, 0x01, 0x01);
	chipio_set_control_param(codec, 0x03, 0x03);

	/*
	 * AE-9: Windows CORB Phase 15 — repeat param 0x18 and reset 0x1C.
	 * After output unmutes, Windows sends 0x18=0x14 again and
	 * resets 0x1C back to 0 (routing commit complete).
	 */
	chipio_set_control_param(codec, 0x18, 0x14);
	...
	/* DIAG: is 8051 alive before asi_setup? */
	{
		unsigned int st = snd_hda_codec_read(codec,
			WIDGET_CHIP_CTRL, 0, 0xF01, 0);
		codec_info(codec,
	...
	/* DIAG: is 8051 alive after asi_setup? */
	{
		unsigned int st = snd_hda_codec_read(codec,
			WIDGET_CHIP_CTRL, 0, 0xF01, 0);
		unsigned int dp = snd_hda_codec_read(codec,
	 ...
	 * 0x18 to verify exram offsets and router register contents.
	 * chipio_8051_read_exram() does not lock the mutex — safe under it.
	 * chipio_read() locks internally — use write_address+read_data directly.
	 */
	{
		unsigned int off_05, off_0c, off_11, off_18;
		unsigned int val, base_addr;
		int j;

		mutex_lock(&spec->chipio_mutex);

		chipio_8051_read_exram(codec, 0x157d, &off_05);
		...
		/* Dump 12 router entries for stream 0x0c (DSP→DAC, 12ch) */
		if (off_0c != 0xff) {
			base_addr = 0x190000 + off_0c * 4;
			for (j = 0; j < 12; j++) {
		...
		/* Dump 6 router entries for stream 0x18 (DSP→I2S, 6ch) */
		if (off_18 != 0xff) {
			base_addr = 0x190000 + off_18 * 4;
			for (j = 0; j < 6; j++) {
				chipio_write_address(codec, base_addr + j * 4);
				chipio_read_data(codec, &val);
				codec_info(codec,
		...
		/* Also dump stream 0x05 (HDA→DSP, 6ch) */
		if (off_05 != 0xff) {
			base_addr = 0x190000 + off_05 * 4;
			for (j = 0; j < 6; j++) {
				chipio_write_address(codec, base_addr + j * 4);
				chipio_read_data(codec, &val);
				codec_info(codec,
	...
	{
		unsigned int ctrl_05 = 0, ctrl_0c = 0, ctrl_18 = 0;
	...
	/* DIAG: check 0x0c before effects loop */
	{
	...
	/*
	 * AE-9: use the standard pe_switch_set path like all other cards.
	 * With PE=0, this sends FLOAT_ZERO to 0x80/0x04 (no effects,
	 * no SBX, no Direct Mode override — just neutral passthrough).
	 */
	ca0132_pe_switch_set(codec);
	...
	ca0113_mmio_gpio_set(codec, 3, false);
	/*
	 * AE-9: Windows MMIO trace shows 0x1e here (not 0x37).
	 * These are ES9038B volume/filter preset registers.
	 */
	...
	/*
	 * AE-9 external DAC power-on sequence (from Creative INF):
	 *   GPIO 5 = ExternalDACReset_Pin: true = release reset = DAC ON
	 *   GPIO 4 = HPAMP_SHDNPin: true = headphone amp ON
	 * Must be set AFTER ae9_post_dsp_asi_setup().
	 *
	 * NOTE: 0x320 is a command register, not a bitmap.
	...
	/*
	 * AE-9: Second PLL + CA0113 engine reset + full re-handshake.
	 *
	 * VFIO trace 27 mars 2026 (Direct Mode) shows Windows does a
	 * SECOND complete reset at t+10s after boot, right before the
	 * ACM I2C init and GPIO4 (relay click). Without this, the I2S
	 * clock to the ACM never starts and the relay never clicks.
	 *
	 * Sequence from trace t=381.008 to t=381.738:
	 *   1. Pre-PLL DAC commands (group 0x48: reg 0x13, 0x14, 0x1d)
	 *   2. GPIO3+0 ON, GPIO3 OFF
	 *   3. PLL reset: 0x304=0xff ×9, 0x100=0x0 ×4
	 *   4. Engine reset: 0x86c=0 → 0x800=0x6b → 0x86c=1
	 *   5. New handshake: sync + mode3 + token
	 *   6. ~30 post-handshake DAC commands (groups 0x48 + 0x49)
	 *   7. I2C mini bus re-init + ACM I2C packets
	 *   8. GPIO4 → relay click
	 */
	/* DIAG: is 8051 alive before second reset? */
	{
		unsigned int st = snd_hda_codec_read(codec,
			WIDGET_CHIP_CTRL, 0, 0xF01, 0);
		codec_info(codec,
	...
	codec_info(codec, "AE-9: starting second PLL + engine reset\n");
	{
		...
		/* Step 1: Pre-PLL DAC commands with sync (group 0x48) */
		ca0113_mmio_command_set(codec, 0x48, 0x13, 0xff);
		ca0113_mmio_command_set(codec, 0x48, 0x14, 0x7f);
		ca0113_mmio_command_set(codec, 0x48, 0x1d, 0x80);

		/* Step 2: GPIO sequence from trace */
		ca0113_mmio_gpio_set(codec, 3, true);
		ca0113_mmio_gpio_set(codec, 0, true);  /* 0x103 */
		ca0113_mmio_gpio_set(codec, 3, false);  /* 0x003 → 0x3 */

		/* Step 3: PLL reset — exact pattern from VFIO trace */
		writeb(0xff, mem + 0x304);
		writeb(0xff, mem + 0x304);
		writeb(0xff, mem + 0x304);
		writeb(0xff, mem + 0x304);
		...
		msleep(7);  /* trace shows 7ms gap */
		writel(0x6b, mem + 0x800);
		writel(0x01, mem + 0x86c);
		msleep(16); /* trace shows 16ms gap */

		/* Step 5: New handshake */
		writel(0x6b, mem + 0x800);
		writel(0x48, mem + 0x804);
		writel(0x00800005, mem + 0x20c);

		/* GPIO5 in the middle of handshake (from trace) */
		ca0113_mmio_gpio_set(codec, 5, true);
		...
		/* Re-sync for post-handshake commands */
		writel(0x00800002, mem + 0x20c);
		writel(0x00, mem + 0x210);
		writel(0x7e, mem + 0x210);
		writel(0x5a, mem + 0x210);
		...
		/* Step 6: Post-handshake DAC commands (group 0x48) */
		ca0113_mmio_command_set(codec, 0x48, 0x07, 0x81);
		ca0113_mmio_command_set(codec, 0x48, 0x1d, 0x40);
		ca0113_mmio_command_set(codec, 0x48, 0x01, 0xc0);
		ca0113_mmio_command_set(codec, 0x48, 0x0a, 0x02);

		/* type2 commands for 0x0b */
		ca0113_mmio_command_set_type2(codec, 0x48, 0x0b, 0x00);
		ca0113_mmio_command_set(codec, 0x48, 0x0b, 0x20);
		...
		/* type2 for 0x0e, then value */
		ca0113_mmio_command_set_type2(codec, 0x48, 0x0e, 0x00);
		ca0113_mmio_command_set(codec, 0x48, 0x0e, 0x8a);

		/* type2 for 0x07, then value */
		ca0113_mmio_command_set_type2(codec, 0x48, 0x07, 0x00);
		ca0113_mmio_command_set(codec, 0x48, 0x07, 0x81);
		...
		/* Post-handshake DAC commands (group 0x49 = ES9038PRO) */
		ca0113_mmio_command_set(codec, 0x49, 0x18, 0xff);
		ca0113_mmio_command_set(codec, 0x49, 0x19, 0xff);
		ca0113_mmio_command_set(codec, 0x49, 0x1a, 0x00);
		ca0113_mmio_command_set(codec, 0x49, 0x1b, 0x70);
	...
	/* DIAG: is 8051 alive after second reset? */
	{
		unsigned int st = snd_hda_codec_read(codec,
			WIDGET_CHIP_CTRL, 0, 0xF01, 0);
		unsigned int dp = snd_hda_codec_read(codec,
	 ...
	 *
	 * Windows sends {0xff,0xff,0xff,0x7f} for the HP gain registers
	 * 0x48/0x11-0x14. The default ae5_headphone_gain_val=0 selects
	 * "Low" preset {0xff,0x2c,0xf5,0x32} which effectively mutes
	 * channels 2-4. Preset 2 = "High" matches the Windows values.
	 */
	/*
	 * AE-9: Write ASI=0x0F after 2nd PLL reset.
	 * DIAG shows ASI stuck at 0x08 because the 8051 ignores
	 * PARAM writes during ae9_post_dsp_asi_setup (bit 16 set).
	 * After the 2nd reset, bit 16 is cleared and the 8051
	 * accepts params again (dac2port=0xA1 works here).
	 * ASI=0x0F = Normal Mode output routing.
	 */
	{
		unsigned int r1, r2, r3;
		/* Explicit ASI=0x0F with error check on each verb */
		r1 = snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0,
					 0x7E8, 23); /* PARAM_SET = ASI */
		r2 = snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0,
					 0x7E9, 0x0f); /* PARAM_VALUE_SET */
		msleep(20);
		/* Try a second write with PARAM_EX path */
		snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0,
				    0x7EE, 23); /* PARAM_EX_ID_SET */
		r3 = snd_hda_codec_write(codec, WIDGET_CHIP_CTRL, 0,
					 0x7EF, 0x0f); /* PARAM_EX_VALUE_SET */
		codec_info(codec,
			   "AE-9 DIAG: ASI=0x0F write: r1=%d r2=%d r3=%d 0x20c=0x%08x\n",
			   r1, r2, r3,
			   readl(spec->mem_base + 0x20c));
	}
	 ...
	 * GPIO5 (DAC board power) must be refreshed every ~500ms or the
	 * DAC board power-cycles, resetting the ES9038 to its power-on
	 * defaults (automute enabled, format unconfigured) and causing
	 * silence on the next playback. The keepalive is stopped only
...
static void ca0132_mmio_init_ae9(struct hda_codec *codec)
{
	...
	/*
	 * AE-9: I2C bus init — MUST happen here, 29ms after CA0113 reset.
	 * Windows VFIO Phase 3 shows this sequence immediately after
	 * 0x804=0x57, BEFORE the 0x20c/0x1c writes.
	 * Phase 1: NO readl() on 0xC00-0xC7F (Windows never reads these).
	 */
	writel(0x80, mem + 0xC0C);
	writel(0x30, mem + 0xC00);
	writel(0x00, mem + 0xC04);
	writel(0x00, mem + 0xC0C);
	writel(0x03, mem + 0xC0C);
	writel(0x03, mem + 0xC0C);
	writel(0x03, mem + 0xC0C);
	writel(0x01, mem + 0xC08);
	writel(0xf1, mem + 0xC08);
	writel(0x01, mem + 0xC08);
	writel(0xc7, mem + 0xC08);
	writel(0xc1, mem + 0xC08);
	writel(0x80, mem + 0xC04);
...
/*
 * AE-9 uses different register set values than the AE-5/AE-7.
 * Derived from Connor McAdams' Windows driver analysis (2022).
 */
...
static void ae5_register_set(struct hda_codec *codec)
{
	...
	/*
	 * AE-9 uses a different PLL/PMU clock value and skips the
	 * 0x93 direct write that the AE-5/AE-7 require.
	 */
	if (ca0132_quirk(spec) == QUIRK_AE9) {
...
static int ca0132_init(struct hda_codec *codec)
{
	...
	/* D2 ACM-only codec: nothing to init */
	if (!spec || spec->is_d2_acm_only)
		return 0;

	cfg = &spec->autocfg;

	/* ZxR DB Pro board uses its own stripped-down init path. */
	if (ca0132_quirk(spec) == QUIRK_ZXR_DBPRO)
...
static void ca0132_free(struct hda_codec *codec)
{
	...
	/* D2 stub codec (AE-9 ACM board) has no spec - nothing to free. */
	if (!spec)
		return;

	/* D2 ACM-only: minimal cleanup */
	if (spec->is_d2_acm_only) {
		kfree(spec);
		return;
	}

	/* ZxR DB Pro board has its own teardown sequence. */
	if (ca0132_quirk(spec) == QUIRK_ZXR_DBPRO) {
		dbpro_free(codec);
		return;
	}
...
static void ca0132_config(struct hda_codec *codec)
{
	...
	case QUIRK_AE9:
		/*
		 * AE-9 pin layout differs from AE-7:
		 *   0x0f = BackPanel HP Out (ACM board headphone jack, tag=07)
		 *   0x10 = Front HP / Center-LFE (tag=04)
		 *   0x11 = LineMicIn2 / Rear L/R (input, not HP)
		 * unsol_tag_hp must point to 0x0f so jack detect and
		 * ca0132_alt_select_out() route audio to the correct pin.
		 */
		/*
		 * AE-9: NID 0x02 reports no supported rates (param 0x06=0x0)
		 * because the CA0132 DSP handles all sample-rate conversion.
		 * Without AC_WCAP_FORMAT_OVRD the HDA core ignores the rates
		 * declared in ca0132_build_pcms() and falls back to querying
		 * the hardware, getting 0x0 and picking DIV=2 (24kHz), which
		 * causes fmt=0x0011 and a stalled DMA ring buffer.
		 */
		if (ca0132_quirk(spec) == QUIRK_AE9)
			snd_hda_override_wcaps(codec, 0x02,
				get_wcaps(codec, 0x02) | AC_WCAP_FORMAT_OVRD);
		spec->num_outputs = 2;
		spec->out_pins[0] = 0x0B; /* Line out */
		spec->out_pins[1] = 0x0F; /* BackPanel HP Out (ACM headphone) */
		spec->out_pins[2] = 0x10; /* Front Headphone / Center/LFE */
		spec->out_pins[3] = 0x11; /* Rear surround / LineMicIn2 */
		spec->shared_out_nid = 0x2;
		spec->unsol_tag_hp = spec->out_pins[1];       /* 0x0f */
		spec->unsol_tag_front_hp = spec->out_pins[2]; /* 0x10 */
...
static int ca0132_codec_probe(struct hda_codec *codec, const struct hda_device_id *id)
{
	struct ca0132_spec *spec;
	int err;
	...
	case QUIRK_AE9:
		...
		/*
		 * AE-9: initialize the period hrtimer.  The CA0132 DSP
		 * firmware does not generate HDA period interrupts, so
		 * we drive snd_pcm_period_elapsed() from a software timer.
		 * The timer is started/stopped in pcm_prepare/pcm_cleanup.
		 */
		hrtimer_setup(&spec->ae9_period_timer,
			      ae9_period_timer_callback,
			      CLOCK_MONOTONIC, HRTIMER_MODE_REL);
		hrtimer_setup(&spec->ae9_keepalive_timer,
...
static struct hda_codec_driver ca0132_driver = {
	...
	.ops = &(const struct hda_codec_ops) {
